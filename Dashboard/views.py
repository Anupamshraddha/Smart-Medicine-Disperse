...
def dashboard(request):
    """
    Clean, simplified dashboard for your medicine dispenser system.
    Shows:
    - Medicine stats
    - ESP/Blynk status
    - Notifications
    ...
    try:
        ...
        messages.error(request, "Profile missing. Please update your profile.")
    ...
    usage_stats = MedicineUsage.objects.filter(
    ...
    # -------------------------------
    #  ESP32 STATUS (from your utility)
    # -------------------------------
    try:
        esp32_status = get_esp32_status()
    except:
        esp32_status = {"status": "Offline", "last_ping": None}

    # -------------------------------
    #  BLYNK STATUS (Online/Offline, sensor & LED state)
    # -------------------------------
    try:
        ...
        blynk_sensor = requests.get(sensor_url, timeout=2).text
        blynk_led = requests.get(led_url, timeout=2).text
        blynk_online = requests.get(device_status_url, timeout=2).text == "true"
    ...
    # Override minimal ESP status for UI
    esp32_status = {
    ...
    if request.method == "POST":
        ...
        for med_id in selected_ids:
            ...
            Notification.objects.create(
                ...
                message=f"You requested {medicine.name}"
            )
            ...
            AccessLog.objects.create(
                ...
                details=f"User requested {medicine.name}"
            )
